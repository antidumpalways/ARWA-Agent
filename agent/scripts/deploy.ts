...
// Robust repo-root detection (works regardless of whether the user
// cloned as `ParkFlow-Agent/`, `ARWA-Agent/`, etc.). See setup.ts for
// the same helper — DRY: import from a shared module in v0.8.3.
...
// Anchor relative paths under <repo>/agent/keys/ for portability across
// folder renames. Absolute paths pass through unchanged.
function resolveKeyPath(): string {
  const rel = process.env.AGENT_SECRET_KEY_PATH ?? 'agent.pem';
  if (path.isAbsolute(rel)) return rel;
  return path.join(REPO, 'agent', 'keys', path.basename(rel));
}
const SECRET_KEY = resolveKeyPath();
